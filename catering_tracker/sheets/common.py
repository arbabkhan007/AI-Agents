...
# totals row
# ---------------------------------------------------------------------------
_NUMFMT = {"money": "#,##0.00", "money0": "#,##0", "num": "#,##0",
           "qty": "#,##0", "qty1": "#,##0.0", "pct": "0%",
           "center": None, "text": None, "date": "dd mmm yyyy"}
...
def totals_row(bk, key, row, cells, first_col="B", last_col=None,
    ...
    for field, (formula, kind, cached) in cells.items():
        col = ci(bk.col(key, field))
        p = dict(font_size=11, bold=True, font_color=th.white,
                 bg_color=th.primary, valign="vcenter", border=1,
                 border_color=th.primary)
        if kind == "center":
            p["align"] = "center"
        else:
            p["align"] = "right"
        nf = _NUMFMT.get(kind, "#,##0")
        if nf:
            p["num_format"] = nf
        fmt = bk.S.f(**bk.S.base(**p))
    ...
    if last_col:
        ...
        skip = [ci(bk.col(key, f)) for f in cells]
        for c in range(ci(first_col), ci(last_col) + 1):
            if c > span_end and c not in skip:
...
def note_block(bk, key, row, first_col, last_col, lines, title=None):
    """A bordered, wrapped tip box.  Cols may be letters or 0-based ints."""
    ws = bk.ws(key)
    c1 = first_col if isinstance(first_col, int) else ci(first_col)
    c2 = last_col if isinstance(last_col, int) else ci(last_col)
    width = sum((C.WIDTHS.get(key, {}) or {}).get(cl(c), 10)
                for c in range(c1, c2 + 1))
    if title:
        ws.merge_range(r(row), c1, r(row), c2, title, bk.S.section_soft)
        ws.set_row(r(row), 22)
        row += 1
    text = "\n".join(lines)
    from ..styles import wrap_height
    h = wrap_height(text, width, minimum=20)
    ws.merge_range(r(row), c1, r(row), c2, text, bk.S.note)
    ws.set_row(r(row), h)
    return row + 1
