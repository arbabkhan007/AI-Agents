"""
catering_lib - draw pixel-perfect "screenshots" of the Catering Business
Manager using the REAL demo data (catering_tracker.demo.Model), for the
...
emoji the real workbook uses.  The palette mirrors the workbook's Classic
theme (cream / espresso / copper / brass).

This module is a sibling of etsy/screenlib.py (the Christmas engine) and
deliberately leaves that file untouched.
"""
...
from catering_tracker.demo import Model
...
PLUM = "#7A5273"
SOFT = {
    "espresso": "#EDE6DA", "latte": "#EBE1D4", "copper": "#F7E7DB",
    "brass": "#F6EED7", "ok": "#E2EEE0", "warn": "#FBF0D9", "bad":
    "#F7E2E0", "info": "#E2ECF3", "plum": "#F0E6EE", "muted": "#F1ECE4",
}
COLORS = {
    "espresso": ESPRESSO, "latte": LATTE, "copper": COPPER, "brass": BRASS,
...
def blocks_bar(img, box, pct, n=40, fg=LATTE, bg=CARD, gap=4,
               border=BORDER):
...
def sheet_header(img, title, subtitle, y=0, width=1320, pill_home=True):
    ...
    draw_text(img, (44, y + 10), title, F("serif_b", 40), ESPRESSO)
    if pill_home:
        f = F("sans_sb", 17)
...
def section_bar(img, y, text, color=ESPRESSO, width=1320, h=52, size=24,
                x=44, w_pad=36):
    d = ImageDraw.Draw(img)
...
STATUS_COLORS = {
    C.ES_INQUIRY: ("info", "info"), C.ES_QUOTED: ("plum", "plum"),
    C.ES_DEPOSIT: ("warn", "warn"), C.ES_CONFIRMED: ("ok", "ok"),
    C.ES_DONE: ("brass", "brass"), C.ES_CANCEL: ("bad", "bad"),
}
PAY_STATUS_COLORS = {
    C.PS_UNPAID: ("bad", "bad"), C.PS_PART: ("warn", "warn"),
    C.PS_PAID: ("ok", "ok"),
}
STOCK_COLORS = {
    "\U0001F534 Reorder": ("bad", "bad"),
    "\U0001F7E1 Low": ("warn", "warn"),
    "\U0001F7E2 OK": ("ok", "ok"),
}


def draw_table(img, x, y, widths, headers, rows, row_h=42, hdr_h=48,
               hdr_bg=ESPRESSO, size=17, hdr_size=15, pad=12):
...
def _axis_title(img, box, title, size=1.0):
    draw_text(img, ((box[0] + box[2]) / 2.0, box[1] - int(34 * size)), title,
              F("serif_sb", int(23 * size)), ESPRESSO, anchor="ma")


def hbar_chart(img, box, title, items, color=COPPER, fmt="${:,.0f}",
               size=1.0):
    x0, y0, x1, y1 = box
    d = ImageDraw.Draw(img)
...
def line_series(img, box, title, cats, values, color=ESPRESSO, fmt="{:,.0f}",
                size=1.0, width=None):
...
def doughnut_chart(img, box, title, items, size=1.0, center_word="events",
    ...
    draw_text(img, (cx, cy - 26 * size), str(big),
              F("display_b", int(44 * size)), ESPRESSO, anchor="ma")
    ...
    for lbl, v, color in items:
        ...
        draw_text(img, (x1, ly - 1), str(v), F("sans_sb", 16), ESPRESSO,
                  anchor="ra")
        ly += 34
