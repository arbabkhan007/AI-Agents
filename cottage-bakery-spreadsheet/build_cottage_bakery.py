...
for r,row in enumerate(recipes,5):
    ...
    ws_lib.cell(row=r, column=6).value = f"=SUMIF($W$65:$W$500,B{r},$AG$65:$AG$500)"
...
ws4["D24"] = "Rate"
ws4["E24"] = "='Instructions + Setup'!B22"  # FIXED (QC): Hourly Labor Rate is B22 (B19 = Owner name)
...
ws4["B25"] = "='Instructions + Setup'!B25"  # FIXED (QC): Packaging Default $ is B25 (B22 = Labor rate)
...
ws4["A26"] = "Overhead %"
ws4["B26"] = "='Instructions + Setup'!B23/100"  # FIXED (QC): Overhead % is B23 (B20 = Currency symbol -> #VALUE!)
...
ws4["A27"] = "Waste %"
ws4["B27"] = "='Instructions + Setup'!B24/100"  # FIXED (QC): Waste % is B24 (B21 = Sales Tax %)
...
calc_rows = [
    ["Total Monthly Overhead","=C18","SUM of Monthly column","Used for % calc"],  # FIXED (QC): doc text no longer starts with = (was parsed as formula)
    ["Total Annual Overhead","=D18","Monthly x 12","Yearly"],
    ["Daily Overhead (30 days)","=C18/30","Monthly / 30","Per day cost"],
    ["Hourly Overhead (80 hrs/month from Setup)","=IF('Instructions + Setup'!B29=0,0,C18/'Instructions + Setup'!B29)","Monthly / Working Hours","Use in Recipe labor overhead"],  # FIXED (QC): B29=Working Hours (B27 was Fiscal Year date)
    ["Overhead per Order (100 orders/month)","=IF('Instructions + Setup'!B30=0,0,C18/'Instructions + Setup'!B30)","Monthly / Expected Orders","Per order overhead"],  # FIXED (QC): B30=Expected Orders (B28 was Market Goal $)
    ["Expected Monthly Revenue (enter)",2000,"","Enter your expected revenue"],  # FIXED (QC): numeric input, was text
...
# ── QC audit fix: convert ISO text dates to real Excel dates ────────────────
# Text like "2026-06-20" stored as a string makes date arithmetic (F5-E5 etc.)
# return #VALUE! in Excel. Convert to genuine datetime values before saving.
import datetime as _dt

# ── Novality Store standardization (QC rebuild) ─────────────────────────────
# Quick Links implemented like the Church Management System reference:
# real INTERNAL hyperlinks (location-only, no external rel), pointing at 'Sheet'!A1.
def novality_finalize(wb, add_links=True):
    ...
    for ws in wb.worksheets:
        try:
            rows = ws.iter_rows(min_row=1, max_row=100, max_col=16)
        except Exception:
            continue
        for row in rows:
            for c in row:
                ...
                if t.startswith("\u2192"):
                    ...
                    t = t[2:].strip()
                tgt = names.get(t.upper())
                if tgt and tgt != ws.title and c.hyperlink is None:
                    c.hyperlink = _H(ref=c.coordinate, location="'" + tgt + "'!A1")
                    f = c.font
                    c.font = _F(name=f.name, size=f.size or 10, bold=f.bold,
                                color="0563C1", underline="single")

_ISO_DATE = None
def convert_text_dates(_wb):
    import re as _re
    ...
    for _ws in _wb.worksheets:
        for _row in _ws.iter_rows():
            for _c in _row:
                _v = _c.value
                if isinstance(_v, str) and pat.match(_v):
                    try:
                        _c.value = _dt.datetime.strptime(_v, "%Y-%m-%d").date()
                        if _c.number_format in ("General", "@"):
