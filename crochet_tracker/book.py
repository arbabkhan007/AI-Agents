...
except ImportError:                        # vendored 3.2.9 rename
...
class Book(object):
    ...
    def _define_names(self):
        ...
        for key, col in sorted(C.LIST_COLS.items()):
            ...
            self.wb.define_name(
                _name_for_list(key),
                "=OFFSET(%s!$%s$%d,0,0,MAX(1,COUNTA(%s)),1)"
                % (su, col, first, rng))
    ...
    def protect_sheet(self, key):
        """Apply the standard protection options to one sheet."""
        if not self.protect:
            return
        self.sheets[key].protect(self.protect, {
