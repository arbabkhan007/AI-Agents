...
def databar(bk, key, field, color=None, first=None, last=None):
    ...
    bk.cond(key, first or C.ROW_FIRST, col, last or C.last_row(key), col, {
        ...
        "bar_solid": True, "min_type": "min", "max_type": "max"})
...
def totals_row(bk, key, row, cells, first_col="B", last_col=None,
    ...
    """``cells`` = {field: (formula, num_format, cached)}."""
    ws = bk.ws(key)
    th = bk.th
    band = bk.S.f(**bk.S.base(bg_color=th.primary, border=1,
    ...
    value_cols = [ci(bk.col(key, f)) for f in cells]
    span_end = ci(label_span[1]) if label_span else ci(first_col)
    span_start = ci(label_span[0]) if label_span else ci(first_col)
    end = ci(last_col) if last_col else max(value_cols + [span_end])
    for c in range(ci(first_col), end + 1):
        ws.write_blank(r(row), c, None, band)
    if label_span:
        c1, c2 = label_span
        if c1 == c2:
    ...
    for field, (formula, num_format, cached) in cells.items():
        col = ci(bk.col(key, field))
...
def note_block(bk, key, row, first_col, last_col, lines, title=None):
    """A bordered, wrapped tip box.  ``first_col``/``last_col`` are
    0-indexed column numbers."""
    ws = bk.ws(key)
    width = sum((C.WIDTHS.get(key, {}) or {}).get(cl(c), 10)
                for c in range(first_col, last_col + 1))
    if title:
        ws.merge_range(r(row), first_col, r(row), last_col, title,
                       bk.S.section_soft)
        ws.set_row(r(row), 22)
        row += 1
    text = "\n".join(lines)
    ...
    ws.merge_range(r(row), first_col, r(row), last_col, text, bk.S.note)
    ws.set_row(r(row), h)
    return row + 1
...
def chips(bk, key, chips, last_col=None, row=None):
    """Stats pills (ROW_STATS by default). chips = (formula, colour,
    cached, span)."""
    from .. import config as _C
    S, th = bk.S, bk.th
    ws = bk.ws(key)
    last_col = last_col or _C.cols_last(key)
    row = row or _C.ROW_STATS
    col = 1
    ...
    if col <= ci(last_col):
        if col == ci(last_col):
            ws.write_blank(r(row), col, None, S.canvas)
        else:
            ws.merge_range(r(row), col, r(row), ci(last_col), "", S.canvas)
