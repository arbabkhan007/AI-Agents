...
class Styles(object):
    ...
    _KINDS = {
        ...
        "calc_qty":   {"align": "center", "num_format": "#,##0"},
        "calc_qty1":  {"align": "center", "num_format": "#,##0.0"},
        "calc_pct":   {"align": "center", "num_format": "0%"},
        "calc_pct1":  {"align": "center", "num_format": "0.0%"},
    ...
    def cell(self, kind, alt=False, **over):
        ...
        if kind.startswith("calc"):
            # calculated cells: tinted + locked, numbers in the theme colour
            props["bg_color"] = th.primary_soft
            props["font_color"] = th.primary if kind not in (
                "calc", "calc_wrap", "calc_c", "calc_tick") else th.ink
            props["locked"] = True
        else:
